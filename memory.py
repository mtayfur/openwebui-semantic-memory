...
QUERY_ENHANCEMENT_PROMPT = """You are a specialized system for enhancing user queries to retrieve relevant personal memories. Your sole function is to analyze a user's query and output a concise, English-language version that best captures the user's intent for memory retrieval.

## Core Directive
- **Extract Intent**: Focus on the core factual content the user seeks.
- **Remove Chit-Chat**: Eliminate greetings, pleasantries, and conversational filler.
- **Expand Pronouns**: Replace pronouns with likely referents (e.g., "he" → "user's brother").
...
- **Add Explanations**: Do not explain, justify, or comment on the query.
- **Retain Chit-Chat**: Exclude greetings, small talk, or conversational asides.
- **Change Factual Meaning**: Do not invent or speculate beyond the user's input.
...
**Enhanced**: "user and user's sister Paris conference last year topic"

### Example 3
**Original**: "I'm curious about the project we did with Dr. Smith in 2023."
**Enhanced**: "user's project with Dr. Smith in 2023"
...
OUTPUT: Enhanced query only, in English."""
...
class Filter:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized batch processing with size {self._batch_size} (CPU cores: {cpu_count})"
        )
    ...
    async def _get_sentence_model(self) -> SentenceTransformer:
        ...
        async with Filter._model_lock:
            if Filter._global_sentence_model is None:
                try:
                    logger.info(
                        f"Loading sentence transformer model: {self.valves.embedding_model}"
                    )
    ...
    async def _cache_embedding(
        self, text: str, embedding: np.ndarray, user_id: str = None
    ) -> None:
        ...
        async with Filter._cache_lock:
            ...
            if len(user_cache) >= self.CACHE_MAX_SIZE:
                ...
                sorted_entries = sorted(
                    user_cache.items(), key=lambda x: x[1]["timestamp"]
                )
                for old_key, _ in sorted_entries[:entries_to_remove]:
                    del user_cache[old_key]
                logger.debug(
                    f"Cleaned {entries_to_remove} old cache entries for user {cache_user_key}"
                )
    ...
    async def _get_embeddings_batch(
        self, texts: List[str], user_id: str = None
    ) -> List[np.ndarray]:
        ...
        logger.debug(
            f"Batch processing: {len(cached_embeddings)} cached, {len(texts_to_process)} to process"
        )
        ...
        if texts_to_process:
            try:
                ...
                for batch_start in range(0, len(texts_to_process), self._batch_size):
                    batch_end = min(
                        batch_start + self._batch_size, len(texts_to_process)
                    )
                ...
                logger.debug(
                    f"Generated {len(new_embeddings)} new embeddings in batches of {self._batch_size}"
                )
    ...
    async def _get_cache_stats(self, user_id: str = None) -> str:
        ...
        async with Filter._cache_lock:
            ...
            total_cache_size = sum(
                len(cache) for cache in Filter._embedding_cache.values()
            )
            return f"user: {len(user_cache)}, total: {total_cache_size}"

    async def _enhance_query(self, query: str) -> str:
        """Enhance the query using LLM for better semantic retrieval."""
        try:
            enhanced = await self._query_llm(
                QUERY_ENHANCEMENT_PROMPT, query, json_response=False
            )
        ...
        try:
            if "messages" in body and body["messages"]:
                ...
                if is_first_message:
                    ...
                    enhanced_query = await self._enhance_query(user_message)
            ...
            await self._emit_status(
                __event_emitter__, f"🙈 Error retrieving memories: {str(e)}"
            )
        return body
            ...
            if "messages" in body and body["messages"]:
                ...
                if user_message:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Memory processing timed out, continuing without waiting"
                        )
        except Exception as e:
            logger.error(f"Error in outlet: {e}\n{traceback.format_exc()}")
            await self._emit_status(
                __event_emitter__, f"🙈 Error processing memories: {str(e)}"
            )
        return body
    ...
    async def _get_formatted_memories(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Database timeout while retrieving memories for user {user_id}"
            )
            return []
        except Exception as e:
            logger.error(
    ...
    async def _process_user_memories(
        ...
        try:
            ...
            logger.error(
                f"Unexpected error in memory processing: {e}\n{traceback.format_exc()}"
            )
            return []
    ...
    async def get_relevant_memories(
        ...
        filtered_memories = [
            mem for mem in scored_memories if mem["relevance"] >= dynamic_threshold
        ]
        ...
        try:
            ...
            if new_operations and existing_contents:
                try:
                    ...
                    all_embeddings = await self._get_embeddings_batch(
                        all_texts, user_id
                    )
                    ...
                    for op in new_operations:
                        ...
                        for existing_content in existing_contents:
                            normalized_existing_content = self._normalize_text(
                                existing_content
                            )
            ...
            for op_data in ops_to_execute:
                success = await self._execute_single_operation(
                    MemoryOperation(**op_data), user
                )
                if success:
